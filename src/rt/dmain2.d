...
extern (C) void _d_print_throwable(Throwable t)
{
    ...
    version (Windows)
    {
        ...
        static struct CPSink
        {
            ...
            void sink(in char[] s) nothrow
            {
                ...
                int slen = WideCharToMultiByte(codepage, 0,
                        ws.ptr, cast(int)ws.len, null, 0, null, null);
                if (!slen) return;

                auto newPtr = cast(char*)realloc(ptr,
                        (this.len + slen) * char.sizeof);
                if (!newPtr) return;
                ptr = newPtr;
                auto written = WideCharToMultiByte(codepage, 0,
                        ws.ptr, cast(int)ws.len, ptr+len, slen, null, null);
                len += written;
            }

            char* get() { if (ptr) ptr[len] = 0; return ptr; }
        ...
        {
            CPSink buf;
            buf.codepage = GetConsoleOutputCP();
            formatThrowable(t, &buf.sink);

            if (buf.ptr)
            {
                WriteFile(hStdErr, buf.ptr, cast(DWORD)buf.len, null, null);
                buf.free;
            }
            return;
        }
